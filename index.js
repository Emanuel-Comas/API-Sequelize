...
app.get("/productos", async (req, res) => {
    try{
    const allProd = productos.findAll()
...
app.get("productos/:id", async (req, res) => {
    try {
        let productosId = parseInt(req.aprams.id)
        let productoEncontrado = await Producto.findByPk(productosId)
...
app.post("/produstos", (req, res) => {
    try {
        let bodyTemp = ""

        req.on("data", (chuck) => {
            bodyTemp += chuck.toString()
        })
        ...
        req.on("end", async () => {
            ...
            const productoAGuardar = new Producto(re.body)
            await roductoAGuardar.save()
        })
    } catch (error) {
...
try {
    ...
    console.log("Exito en conexion db")
